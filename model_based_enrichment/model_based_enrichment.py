...
class MBEModel:
    ...
    def fit(self, sequences, counts, normalize_classes=True, n_jobs=None, verbose=0):
        ...
        for c in range(n_classes):
            observed = counts[:, c] > 0
            X.append(np.array(Parallel(n_jobs=n_jobs, verbose=verbose)(delayed(self.encoding_fn)(seq) for seq in sequences[observed])))
    ...
    def log_enrichment(self, sequences, numerator_index=1, denominator_index=0, n_jobs=None, verbose=0):
        """Predict log-enrichment (aka. log-density ratio) values for the
        provided sequences (strings) using trained model."""
        X = np.array(Parallel(n_jobs=n_jobs, verbose=verbose)(delayed(self.encoding_fn)(seq) for seq in sequences))
...
class MBETensorflowModel(MBEModel):
    ...
    def fit(self, sequences, counts, ids=None, batch_size=1024, shuffle=True, shuffle_buffer=int(1e4),
        ...
        dataset = get_classification_dataset(sequences, counts, ids, self.encoding_fn, batch_size,
                                             shuffle, shuffle_buffer, seed, flatten, tile, n_jobs, verbose)
    ...
    def log_enrichment(self, sequences, numerator_index=1, denominator_index=0, batch_size=1024, flatten=True,
        ...
        """Predict log-enrichment (aka. log-density ratio) values for the
        provided sequences (strings) using trained model."""
        dataset = get_sequence_dataset(sequences, self.encoding_fn, batch_size=batch_size,
                                       shuffle=False, flatten=flatten, n_jobs=n_jobs, verbose=verbose)
...
def get_classification_dataset(sequences, counts, ids, encoding_fn,
                               batch_size=1024, shuffle=True, shuffle_buffer=int(1e4),
                               seed=None, flatten=True, tile=True, n_jobs=None, verbose=0):
    ...
    X = np.array(Parallel(n_jobs=n_jobs, verbose=verbose)(delayed(encoding_fn)(seq) for seq in sequences.iloc[ids]))
    counts = counts[ids]
...
def get_sequence_dataset(sequences, encoding_fn, ids=None,
                         batch_size=1024, shuffle=True, shuffle_buffer=int(1e4),
                         seed=None, flatten=True, n_jobs=None, verbose=0):
...
def parse_csv(fname, seq_column='seq', count_columns=None, groundtruth_column=None, filter_unobserved=True):
    """Returns list of sequences (string) and n_sequences x n_conditions array of read counts (integers)
    parsed from the provided CSV file. Optionally returns ground truth values stored in provided CSV."""
    data_df = pd.read_csv(fname)
    seqs = data_df[seq_column] # Read sequences as strings.
    if count_columns is None or len(count_columns) < 2:
        raise AssertionError("Must provide counts for at least two conditions using 'count_columns'.", count_columns)
    counts = np.column_stack([data_df[c] for c in count_columns]) # n_sequences x n_conditions array of read counts (integers).
    mask = np.sum(counts, axis=1) > 0 if filter_unobserved else [True] * len(data_df)
    seqs, counts = seqs[mask], counts[mask]
    if groundtruth_column is not None:
        groundtruth = data_df[groundtruth_column].values[mask] # Groundtruth property/enrichment values for evaluation.
        return seqs, counts, groundtruth
    return seqs, counts
